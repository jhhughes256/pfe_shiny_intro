...
app_server <- function(input, output) {
  ...
  Rsimdf <- reactive({
  # Application inputs - updated to include the new NID input
    NID <- input$NID
  ...
  # Application inputs - updated to include the new NID & logscale input
    NID <- input$NID
    DOSEDUR <- input$DOSEDUR
  ...
  # Add conditional ggplot code to be added to the plot object when 
  #   input$logscale == TRUE (i.e. when checkbox is ticked)
    if (logscale) {
      p <- p + scale_y_log10()
    }
    p
...
# The when function from purrr is used at the end of the table definition to
#   remove the value under 90% PI if NID == 1
